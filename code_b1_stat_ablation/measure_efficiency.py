...
def measure_inference_time_ms(model, data_loader, device, warmup=10, test_batches=100):
    ...
    return (end - start) * 1000 / actual  # ms / batch
...
def build_config(use_b1=False):
    return {
        ...
        "dataset": "SMD",
        "mode": "train",
        "data_path": "./dataset/SMD",   # 如果你的 SMD 路径不是这个，要改这里
...
def main():
    print("Building AT ...")
    solver_at = Solver(build_config(use_b1=False))
    ...
    print("\nBuilding Proposed Method ...")
    solver_prop = Solver(build_config(use_b1=True))

    prop_params = count_trainable_params_m(solver_prop.model)
    prop_flops = measure_flops_g(solver_prop.model, (1, 100, 38), device)
    prop_infer_time = measure_inference_time_ms(solver_prop.model, solver_prop.test_loader, device)

    print("\n===== Proposed Method =====")
    print(f"Params (M): {prop_params:.6f}")
    print(f"FLOPs (G): {prop_flops:.6f}")
    print(f"Inference Time / Batch (ms): {prop_infer_time:.6f}")
