...
class AnomalyTransformer(nn.Module):
    def __init__(self, win_size, enc_in, c_out, d_model=512, n_heads=8, e_layers=3, d_ff=512,
                 ...
                 use_b1=False, ma_window=5, alpha_base=1.2, alpha_range=0.3,
                 alpha_hidden=-1, stat_type='mean_max'):
    ...
    def b1_enhance(self, x):
        ...
        mean_abs = abs_res.mean(dim=1)                   # [B, C]
        max_abs = abs_res.max(dim=1).values             # [B, C]
        std_abs = abs_res.std(dim=1, unbiased=False)    # [B, C]

        if self.stat_type == 'mean':
            stat = mean_abs
        elif self.stat_type == 'max':
            stat = max_abs
        elif self.stat_type == 'mean_max':
            stat = torch.cat([mean_abs, max_abs], dim=-1)
        elif self.stat_type == 'mean_std':
            stat = torch.cat([mean_abs, std_abs], dim=-1)
        else:
