...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        # ===== B1 default params =====
        if not hasattr(self, 'use_b1'):
            self.use_b1 = False
        if not hasattr(self, 'ma_window'):
            self.ma_window = 5
        if not hasattr(self, 'alpha_base'):
            self.alpha_base = 1.2
        if not hasattr(self, 'alpha_range'):
            self.alpha_range = 0.3
        if not hasattr(self, 'alpha_hidden'):
            self.alpha_hidden = -1

        # ===== score fusion default params =====
        if not hasattr(self, 'use_score_fusion'):
            self.use_score_fusion = False
        if not hasattr(self, 'lambda_evi'):
            self.lambda_evi = 0.005
        if not hasattr(self, 'score_eps'):
            self.score_eps = 1e-4
    ...
    def moving_avg_for_score(self, x):
        """
        x: [B, L, C]
        return bg: [B, L, C]
        """
    ...
    def fuse_with_residual_score(self, input_x, base_score):
        ...
        bg = self.moving_avg_for_score(input_x)
        residual = input_x - bg
        ...
        evi_norm = (evi_score - mu) / (std + self.score_eps)
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):
            ...
            cri = metric * loss  # [B, L]
            cri = self.fuse_with_residual_score(input, cri)
