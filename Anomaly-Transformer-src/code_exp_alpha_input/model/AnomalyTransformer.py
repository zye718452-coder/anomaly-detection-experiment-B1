...
class AnomalyTransformer(nn.Module):
    def __init__(self, win_size, enc_in, c_out, d_model=512, n_heads=8, e_layers=3, d_ff=512, dropout=0.0,
        ...
        # sample-dependent channel-wise alpha generator
        # input:  [B, C]
        # output: [B, C]
    ...
    def get_alpha(self, x_res):
        ...
        """
        # summarize residual strength over time
        res_stat = x_res.abs().mean(dim=1)   # [B, C]
    ...
    def forward(self, x):
        ...
        alpha = alpha_bc.unsqueeze(1)           # [B, 1, C] for broadcast
