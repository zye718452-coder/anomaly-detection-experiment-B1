...
class AnomalyTransformer(nn.Module):
    def __init__(self, win_size, enc_in, c_out, d_model=512, n_heads=8, e_layers=3, d_ff=512, dropout=0.0,
        ...
        # ===== learnable channel-wise alpha =====
        self.alpha_max = 2.0
        alpha_init = 1.2

        # alpha = alpha_max * sigmoid(alpha_logits)
        # initialize alpha near 1.2
        ratio = alpha_init / self.alpha_max
        init_logit = math.log(ratio / (1.0 - ratio))

        # one learnable alpha for each channel
        self.alpha_logits = nn.Parameter(torch.ones(enc_in) * init_logit)
    ...
    def forward(self, x):
        # residual-enhanced input with learnable channel-wise alpha
