...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        # score fusion 超参数
        self.lambda_evi = 0.001
        self.evi_eps = 1e-4

        # train-time 弱一致性正则，先取很小
        self.beta_evi = 1e-4
    ...
    def _normalize_evidence_score(self, e0):
        """
        e0: [B, L, C]
        return:
            evi_score_pos: [B, L]
        """
        evi_score = torch.mean(torch.abs(e0), dim=-1)              # [B, L]
        evi_mean = torch.mean(evi_score, dim=1, keepdim=True)      # [B, 1]
        evi_std = torch.std(evi_score, dim=1, keepdim=True)        # [B, 1]

        evi_score = (evi_score - evi_mean) / (evi_std + self.evi_eps)
        evi_score = torch.relu(evi_score)
        return evi_score
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            output, series, prior, sigmas, e0 = self.model(input)
            ...
            evi_score = self._normalize_evidence_score(e0)
            evi_reg = torch.mean(evi_score)

            loss_1.append((rec_loss - self.k * series_loss + self.beta_evi * evi_reg).item())
            loss_2.append((rec_loss + self.k * prior_loss + self.beta_evi * evi_reg).item())
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                loss1 = rec_loss - self.k * series_loss + self.beta_evi * evi_reg
                loss2 = rec_loss + self.k * prior_loss + self.beta_evi * evi_reg
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):
            ...
            cri = metric * loss + self.lambda_evi * evi_score
